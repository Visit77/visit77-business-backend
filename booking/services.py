...
from booking.booking_services.invoice_charges import (
    booking_charge_dimensions,
    calculate_invoice_charges,
    charge_totals,
)
...
def _meal_plan_prices(link, meal_plan, guest_market):
    ...
    if guest_market == RatePlan.GuestMarket.FOREIGN:
        return meal_plan.foreign_base_price, meal_plan.foreign_usd_display_price
    return meal_plan.local_base_price, meal_plan.local_usd_display_price


def _resolve_booking_meal_plan(room_type, requested):
    meal_plan_id = requested.get("meal_plan_id")
    meal_plan_link_id = requested.get("meal_plan_link_id")
    if meal_plan_id and meal_plan_link_id:
    ...
    if meal_plan_link_id:
        ...
        return link, link.meal_plan
    link = links.filter(is_default=True, is_included=True).order_by("id").first()
    return (link, link.meal_plan) if link else (None, None)


def _resolve_booking_meal_plans(room_type, requested):
    """Resolve multiple standalone plans while retaining legacy single-plan inputs."""
    meal_plan_ids = requested.get("meal_plan_ids") or []
    if meal_plan_ids:
        if requested.get("meal_plan_id") or requested.get("meal_plan_link_id"):
            raise ValidationError({
                "rooms": "Send only one of meal_plan_ids, meal_plan_id, or meal_plan_link_id."
            })
        if len(meal_plan_ids) != len(set(meal_plan_ids)):
            raise ValidationError({"rooms": "Duplicate meal plans are not allowed."})
        plans = {
            plan.id: plan
...
def _meal_plan_booking_snapshot(link, meal_plan, guest_market, room_type, display_currency=None):
    if not meal_plan:
        return {}
    base_price, usd_display_price = _meal_plan_prices(link, meal_plan, guest_market)
    selected_display_currency = display_currency or room_type.hotel.base_currency
    included = bool(link and link.is_included)
    charge_base_price = Decimal("0") if included else base_price
    charge_usd_display_price = Decimal("0") if included and usd_display_price is not None else usd_display_price
    return {
        "id": link.id if link else meal_plan.id,
        "meal_plan_link_id": link.id if link else None,
        "meal_plan_id": meal_plan.id,
        "core_meal_plan_id": meal_plan.core_meal_plan_id,
        "name": meal_plan.name,
        "description": meal_plan.description,
        ...
        "is_guest_selectable": True if not link else link.is_guest_selectable,
        "pricing_mode": link.pricing_mode if link else meal_plan.package_pricing_mode,
...
def _meal_plan_nightly_total(link, meal_plan, guest_market, quantity):
    if not meal_plan or (link and link.is_included):
        return Decimal("0")
    base_price, _usd_display_price = _meal_plan_prices(link, meal_plan, guest_market)
...
def availability_for_hotels(
    ...
    for room_type in room_types:
        ...
        if room_plans and available > 0:
            ...
            base_payload.update({
                "extra_bed_available": extra_bed_config["extra_bed_available"],
                "extra_bed_quantity": extra_bed_config["extra_bed_quantity"],
            })
...
def estimate_booking(data):
    ...
    for requested in data["rooms"]:
        ...
        total_guest_capacity += room_type.max_occupancy * quantity
        ...
        resolved_meal_plans = _resolve_booking_meal_plans(room_type, requested)
        meal_plan_snapshots = [
            _meal_plan_booking_snapshot(link, plan, guest_market, room_type)
            for link, plan in resolved_meal_plans
        ]
        meal_plan_snapshot = meal_plan_snapshots[0] if meal_plan_snapshots else {}
        ...
        for day in dates:
            ...
            meal_plan_night_total = sum((
                _meal_plan_nightly_total(link, plan, guest_market, quantity)
                for link, plan in resolved_meal_plans
            ), Decimal("0"))
        ...
        for (meal_plan_link, meal_plan), snapshot in zip(resolved_meal_plans, meal_plan_snapshots):
            plan_stay_total = _meal_plan_nightly_total(
                meal_plan_link, meal_plan, guest_market, quantity,
            ) * len(dates)
            snapshot["nightly_total"] = str(
                _meal_plan_nightly_total(meal_plan_link, meal_plan, guest_market, quantity)
            )
            snapshot["stay_total"] = str(plan_stay_total)
            summary_items.append({
                "type": "meal_plan",
                "label": f"{meal_plan.name} x {quantity} Room{'s' if quantity != 1 else ''} x {len(dates)} Night{'s' if len(dates) != 1 else ''}",
                "room_type_id": room_type.id,
                "core_room_type_id": room_type.core_room_type_id,
                "rate_plan_id": rate_plan.id,
                "meal_plan_link_id": meal_plan_link.id if meal_plan_link else None,
                "meal_plan_id": meal_plan.id,
                "quantity": quantity,
                "amount": plan_stay_total,
                "formatted_amount": format_money(plan_stay_total, hotel.base_currency),
            })
        ...
        rooms.append({
            ...
            "meal_plans": meal_plan_snapshots,
    ...
    charge_config = (
        hotel.pms_invoice_charges
        if str(data.get("source", Booking.Source.OTA)).lower() == Booking.Source.PMS
        else hotel.ota_invoice_charges
    )
    charge_snapshot = calculate_invoice_charges(
        charge_config,
        grand_total,
        nights=len(dates),
        guest_count=occupancy,
        room_count=selected_room_count,
    )
    service_charge_total, tax_total = charge_totals(charge_snapshot)
    for category in ("service_charges", "taxes"):
        for charge in charge_snapshot[category]:
            if Decimal(charge["amount"]):
                summary_items.append({
                    "type": "service_charge" if category == "service_charges" else "tax",
                    "label": charge["title"],
    ...
    return {
        ...
        "tax_total": tax_total,
        "invoice_charges": charge_snapshot,
        "grand_total": grand_total + service_charge_total + tax_total,
        ...
        "formatted_grand_total": format_money(grand_total + service_charge_total + tax_total, hotel.base_currency),
...
def create_booking(data, idempotency_key=None):
    ...
    for requested in data["rooms"]:
        ...
        meal_plan_link, meal_plan = resolved_meal_plans[0] if resolved_meal_plans else (None, None)
        meal_plan_snapshot = meal_plan_snapshots[0] if meal_plan_snapshots else {}
        breakfast_snapshot, breakfast_unit_price = _resolve_booking_breakfast(room_type, requested, guest_market)
        ensure_preference_capacity(
            room_type,
        ...
        booking_room = BookingRoom.objects.create(
            ...
            meal_plan_snapshots=meal_plan_snapshots,
        ...
        for day in dates:
            ...
            meal_plan_stay_total += meal_plan_night_total
            breakfast_stay_total += breakfast_night_total
        for (link, plan), snapshot in zip(resolved_meal_plans, meal_plan_snapshots):
            plan_nightly_total = _meal_plan_nightly_total(link, plan, guest_market, quantity)
            snapshot["nightly_total"] = str(plan_nightly_total)
            snapshot["stay_total"] = str(plan_nightly_total * len(dates))
        ...
        booking_room.meal_plan_snapshots = meal_plan_snapshots
        booking_room.meal_plan_snapshot = meal_plan_snapshots[0] if meal_plan_snapshots else {}
        booking_room.save(update_fields=[
    ...
    if uses_common_guest_counts:
        ...
        for booking_room in created_booking_rooms:
            capacity = booking_room.room_type.max_occupancy * booking_room.quantity
    ...
    charge_config = data.get("_invoice_charges", (
        hotel.ota_invoice_charges
        if booking.source == Booking.Source.OTA else hotel.pms_invoice_charges
    ))
    booking.invoice_charge_snapshot = calculate_invoice_charges(
        charge_config,
        room_total + add_on_total,
        nights=len(dates),
        guest_count=occupancy,
        room_count=sum(room.quantity for room in created_booking_rooms),
    )
    booking.service_charge_total, booking.tax_total = charge_totals(
        booking.invoice_charge_snapshot,
    )
    booking.grand_total = (
        room_total + add_on_total + booking.service_charge_total
        + booking.tax_total - booking.discount_total
    )
    booking.cancellation_policy_snapshot = policy_snapshot
    booking.save(update_fields=[
        "currency", "room_total", "add_on_total", "service_charge_total",
        "tax_total", "invoice_charge_snapshot", "grand_total",
...
def create_invoice(
    booking, invoice_type, lines, tax_total=Decimal("0"), discount_total=Decimal("0"),
    note="", add_to_booking_total=False, charge_scope=None, charge_snapshot=None,
    apply_config=False,
):
    charge_scope = charge_scope or Invoice.ChargeScope.PMS
    normalized_lines = []
    subtotal = Decimal("0")
    for item in lines:
    ...
    if apply_config:
        config = (
            booking.hotel.ota_invoice_charges
            if charge_scope == Invoice.ChargeScope.OTA else booking.hotel.pms_invoice_charges
        )
        charge_snapshot = calculate_invoice_charges(
            config, subtotal, **booking_charge_dimensions(booking),
        )
        service_total, tax_total = charge_totals(charge_snapshot)
        for index, charge in enumerate(charge_snapshot.get("service_charges", [])):
            amount = Decimal(charge["amount"])
            if amount:
                item = {
                    "description": charge["title"], "quantity": 1, "unit_price": amount,
                    "metadata": {"line_type": "invoice_service_charge", "charge_index": index},
                }
                normalized_lines.append((item, Decimal("1"), amount, amount))
        subtotal += service_total
    charge_snapshot = charge_snapshot or {"taxes": [], "service_charges": []}
...
def _booking_charge_lines(booking):
    ...
    for room in booking.rooms.select_related("room_type", "meal_plan_link__meal_plan").prefetch_related("nights"):
        ...
        meal_plan_snapshots = room.meal_plan_snapshots or (
            [room.meal_plan_snapshot] if room.meal_plan_snapshot else []
        )
        for meal_plan_snapshot in meal_plan_snapshots:
            meal_plan_id = meal_plan_snapshot.get("meal_plan_id")
            if not meal_plan_id and room.meal_plan_link_id:
                meal_plan_id = room.meal_plan_link.meal_plan_id
            if not meal_plan_id:
                continue
            linked_meal_plan_name = (
                room.meal_plan_link.meal_plan.name if room.meal_plan_link_id else "Meal Plan"
            )
    ...
    for index, charge in enumerate(booking.invoice_charge_snapshot.get("service_charges", [])):
        amount = Decimal(charge["amount"])
        if amount:
            lines.append({
...
def ensure_initial_invoice(booking):
    ...
    return create_invoice(
        ...
        charge_scope=(
            Invoice.ChargeScope.OTA
            if booking.source == Booking.Source.OTA else Invoice.ChargeScope.PMS
        ),
...
def sync_initial_invoice(booking):
    ...
    invoice.charge_scope = (
        Invoice.ChargeScope.OTA
        if booking.source == Booking.Source.OTA else Invoice.ChargeScope.PMS
    )
    invoice.charge_snapshot = booking.invoice_charge_snapshot
    invoice.total = subtotal + booking.tax_total - booking.discount_total
    invoice.save(update_fields=[
        "subtotal", "tax_total", "discount_total", "charge_scope",
        "charge_snapshot", "total", "updated_at",
    ])
...
def _charge_line_key(line):
    ...
    identifiers = {
        ...
        "invoice_other_charge": metadata.get("charge_index"),
        "invoice_service_charge": metadata.get("charge_index"),
    }
...
def _replace_invoice_lines(
    ...
    if apply_config:
        config = (
            ...
            if invoice.charge_scope == Invoice.ChargeScope.OTA
            else invoice.booking.hotel.pms_invoice_charges
        )
        charge_snapshot = calculate_invoice_charges(
            config, subtotal, **booking_charge_dimensions(invoice.booking),
        )
        service_total, tax_total = charge_totals(charge_snapshot)
        lines = list(lines) + [
            {
                "description": charge["title"], "quantity": 1,
    ...
    invoice.charge_snapshot = charge_snapshot
    invoice.total = invoice.subtotal + invoice.tax_total - invoice.discount_total
    invoice.save(update_fields=["subtotal", "tax_total", "discount_total", "charge_snapshot", "total", "updated_at"])
    return _sync_invoice_status(invoice)
...
def sync_booking_charge_invoices(booking):
    ...
    if (
        initial.status != Invoice.Status.PAID
        and initial.charge_scope == Invoice.ChargeScope.PMS
    ):
    ...
    if supplemental:
        return _replace_invoice_lines(supplemental, additional_lines, apply_config=True)
    return create_invoice(
        booking,
...
def _manual_invoice_amount(rule, base_amount):
    if not rule:
        return Decimal("0")
    value = Decimal(str(rule["value"]))
    if rule["mode"] == "percentage":
        return (base_amount * value / Decimal("100")).quantize(
            Decimal("0.01"), rounding=ROUND_HALF_UP,
        )
    return value


def apply_check_in_invoice_adjustments(booking, adjustments=None):
    """Replace manual rows on the booking's single editable PMS invoice."""
    editable = list(
        booking.invoices.select_for_update().filter(
            charge_scope=Invoice.ChargeScope.PMS,
    ...
    if len(editable) > 1:
        raise ValidationError({
            "invoice": "This booking has more than one editable PMS invoice. Resolve the duplicate invoices first."
        })
    invoice = editable[0] if editable else None

    if adjustments is None:
        existing_manual = (
            (invoice.charge_snapshot or {}).get("manual_adjustments")
            if invoice else None
        )
        if not existing_manual:
            return invoice
        adjustments = existing_manual.get("input") or {}

    if invoice is None:
        invoice = Invoice.objects.create(
            booking=booking,
            invoice_type=Invoice.Type.EXTRA_SERVICE,
            charge_scope=Invoice.ChargeScope.PMS,
            currency=booking.currency,
            note="Additional stay charges",
        )

    invoice = Invoice.objects.select_for_update().prefetch_related("receipts").get(pk=invoice.pk)
    if invoice.charge_scope != Invoice.ChargeScope.PMS or invoice.status not in {
    ...
    manual_charge_total = sum(
        (Decimal(str(item["amount"])) for item in charges), Decimal("0"),
    )
    calculation_base = system_subtotal + manual_charge_total
    discount_amount = _manual_invoice_amount(discount, calculation_base)
    adjustment_amount = _manual_invoice_amount(adjustment, calculation_base)
    adjustment_signed = (
        -adjustment_amount
        if adjustment and adjustment["operation"] == "subtract"
        else adjustment_amount
    )
    manual_tax_total = sum(
    ...
    new_lines = [
        InvoiceLine(
            ...
            metadata={"line_type": "manual_charge", "manual": True},
        )
        for item in charges
    ]
    if adjustment and adjustment_amount:
        new_lines.append(InvoiceLine(
            invoice=invoice,
            description=adjustment.get("description") or "Adjustment",
            quantity=Decimal("1"),
            unit_price=adjustment_signed,
            total=adjustment_signed,
            metadata={
    ...
    snapshot["taxes"] = auto_taxes + manual_tax_rows
    snapshot["manual_adjustments"] = {
        "input": {
            "charges": [
            ...
            "discount": (
                {"mode": discount["mode"], "value": str(discount["value"])}
        ...
        "calculation_base": str(calculation_base),
        "manual_charge_total": str(manual_charge_total),
        "discount_amount": str(discount_amount),
        "adjustment_amount": str(adjustment_signed),
    ...
    if invoice.total < invoice.paid_amount:
        raise ValidationError({
            "invoice_adjustments": (
                "Updated invoice total cannot be lower than the amount already paid. "
                "Refund or correct the payment first."
            )
        })
    if invoice.total < 0:
        raise ValidationError({"invoice_adjustments": "Invoice total cannot be negative."})
    invoice.charge_snapshot = snapshot
    invoice.save(update_fields=[
        "subtotal", "discount_total", "tax_total", "total", "charge_snapshot", "updated_at",
    ])
    invoice = _sync_invoice_status(invoice)

    booking.grand_total += invoice.total - previous_total
...
def create_walk_in_booking(data, idempotency_key=None, core_business_id=None, check_in_immediately=True):
    requested_rooms = data.get("rooms") or [{
        ...
        "meal_plan_ids": data.get("meal_plan_ids") or [],
...
def update_reservation_for_check_in(booking, data, *, replace_payment=False):
    ...
    if requested_rooms is None:
        ...
        for room in existing_rooms:
            requested_rooms.append({
                ...
                "meal_plan_id": (
                    (room.meal_plan_snapshot or {}).get("meal_plan_id")
                    if not room.meal_plan_link_id and not room.meal_plan_snapshots
                    else None
                ),
                "meal_plan_ids": [
                    snapshot.get("meal_plan_id")
                    for snapshot in (room.meal_plan_snapshots or [])
                    if snapshot.get("meal_plan_id")
                ],
    ...
    if booking.invoice_charge_snapshot:
        replacement_payload["_invoice_charges"] = {
            category: [
                {
                    key: charge[key]
                    for key in ("title", "mode", "value", "calculation_basis")
                    if key in charge
                }
                for charge in booking.invoice_charge_snapshot.get(category, [])
                if not charge.get("manual")
            ]
    ...
    if booking.invoice_charge_snapshot:
        replacement.invoice_charge_snapshot = calculate_invoice_charges(
            replacement_payload["_invoice_charges"],
            replacement.room_total + replacement.add_on_total,
            **booking_charge_dimensions(replacement),
        )
        replacement.service_charge_total, replacement.tax_total = charge_totals(
            replacement.invoice_charge_snapshot,
        )
    else:
        replacement.invoice_charge_snapshot = booking.invoice_charge_snapshot
        replacement.service_charge_total = booking.service_charge_total
        replacement.tax_total = booking.tax_total
    replacement.discount_total = booking.discount_total
    replacement.grand_total = (
        replacement.room_total + replacement.add_on_total
        + replacement.service_charge_total + replacement.tax_total - replacement.discount_total
    )
    replacement.save(update_fields=[
        "service_charge_total", "invoice_charge_snapshot", "tax_total",
