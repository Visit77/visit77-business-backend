...
class BookingServiceTests(TestCase):
    ...
    def test_fixed_invoice_charge_calculation_bases_include_nights(self):
        snapshot = calculate_invoice_charges({
            "service_charges": [
                {
                    "title": "Booking Fee",
                    "mode": "fixed",
                ...
                },
                {
                    ...
                    "mode": "fixed",
                    "value": "100",
                    "calculation_basis": "per_guest_per_night",
                },
                {
                    "title": "Room Fee",
                    "mode": "fixed",
            ...
            "taxes": [],
        }, Decimal("1000"), nights=3, guest_count=4, room_count=2)

        self.assertEqual(
            [Decimal(item["amount"]) for item in snapshot["service_charges"]],
            [Decimal("300"), Decimal("1200"), Decimal("600")],
        )
        self.assertEqual(
            [item["multiplier"] for item in snapshot["service_charges"]],
            [3, 12, 6],
        )

    def test_booking_fixed_guest_charge_uses_create_occupancy_not_guest_records(self):
        self.hotel.ota_invoice_charges = {
            "taxes": [],
            "service_charges": [{
                "title": "Guest Fee",
                "mode": "fixed",
        ...
        # The payload has four adults for two nights, while only the default
        # primary Guest record exists. Pricing must use the submitted occupancy.
        self.assertEqual(booking.guests.count(), 1)
        self.assertEqual(booking.service_charge_total, Decimal("800"))
        self.assertEqual(
            booking.invoice_charge_snapshot["service_charges"][0]["multiplier"],
            8,
        )

    def test_pms_booking_uses_pms_invoice_charges(self):
        self.hotel.pms_invoice_charges = {
            "taxes": [{"title": "VAT", "mode": "percentage", "value": "5.00"}],
            "service_charges": [{"title": "Service Charge", "mode": "percentage", "value": "10.00"}],
        }
        ...
        self.assertEqual(booking.service_charge_total, Decimal("32000"))
        ...
        self.assertEqual(
            list(invoice.lines.filter(metadata__line_type="invoice_service_charge").values_list("description", "total")),
    ...
    def test_ota_initial_invoice_and_pms_service_invoice_use_separate_configs(self):
        self.hotel.ota_invoice_charges = {
            "taxes": [{"title": "OTA Tax", "mode": "included", "value": "0"}],
            "service_charges": [{"title": "OTA Service", "mode": "percentage", "value": "5"}],
        }
        self.hotel.pms_invoice_charges = {
            "taxes": [{"title": "PMS Tax", "mode": "percentage", "value": "5"}],
            "service_charges": [{"title": "PMS Service", "mode": "percentage", "value": "10"}],
        }
        self.hotel.save(update_fields=["ota_invoice_charges", "pms_invoice_charges"])

        booking, _ = create_booking(self.payload())
        ota_invoice = booking.invoices.get(invoice_type=Invoice.Type.ROOM_BOOKING)
        self.assertEqual(ota_invoice.charge_scope, Invoice.ChargeScope.OTA)
        self.assertEqual(ota_invoice.charge_snapshot["taxes"][0]["title"], "OTA Tax")
        self.assertEqual(ota_invoice.lines.get(metadata__line_type="invoice_service_charge").total, Decimal("16000"))

        pms_invoice = create_invoice(
            booking,
            Invoice.Type.EXTRA_SERVICE,
            [{"description": "Late checkout", "quantity": 1, "unit_price": "1000"}],
            charge_scope=Invoice.ChargeScope.PMS,
            apply_config=True,
        )
        self.assertEqual(pms_invoice.charge_snapshot["taxes"][0]["title"], "PMS Tax")
        self.assertEqual(pms_invoice.lines.get(metadata__line_type="invoice_service_charge").total, Decimal("100"))
        self.assertEqual(pms_invoice.tax_total, Decimal("50"))
        self.assertEqual(pms_invoice.total, Decimal("1150"))
    ...
    def test_availability_returns_one_record_per_room_type(self):
        ...
        self.assertEqual(available[0]["name"], self.room_type.name)
        self.assertEqual(available[0]["available_rooms"], 6)
        self.assertTrue(available[0]["extra_bed_available"])
        self.assertEqual(available[0]["extra_bed_quantity"], 3)
        self.assertNotIn("availability_variant_id", available[0])
        self.assertNotIn("extra_bed_count", available[0])
...
class BookingApiTests(BookingServiceTests):
    ...
    def test_invoice_charge_setup_prices_booking_and_hides_unconfigured_rows(self):
        ...
        configured = self.client.put(setup_url, {
            "taxes": [
                {"title": "Hidden Tax", "mode": "do_not_show"},
        ...
        self.assertEqual(
            [item["title"] for item in grouped.data["data"]["other_tax"]],
            ["Hidden Tax", "VAT", "Tourism Tax"],
        )
        self.assertEqual(
            [item["title"] for item in grouped.data["data"]["service_charge"]],
            ["Service Charge"],
        )
        self.assertEqual(
            [item["title"] for item in grouped.data["data"]["other_charge"]],
            ["Resort Fee"],
        )
        created_charge = self.client.post(ota_crud_url, {
            "charge_type": "service",
            "charge_kind": "other_charge",
        ...
        self.assertEqual(
            created_charge.data["data"]["calculation_basis"],
            "per_room_per_night",
        )
        invalid_kind = self.client.post(ota_crud_url, {
            "charge_type": "tax",
            "charge_kind": "service_charge",
            "title": "Invalid Tax Kind",
            "mode": "fixed",
            "value": "500",
        }, format="json", **headers)
        self.assertEqual(invalid_kind.status_code, 400, invalid_kind.data)
        self.assertIn("invalid charge kind", str(invalid_kind.data).lower())
        charge_id = created_charge.data["data"]["id"]
        duplicate_name = self.client.post(ota_crud_url, {
            "charge_type": "tax",
            "charge_kind": "other_tax",
            "title": "  cleaning charge  ",
            "mode": "fixed",
            "value": "500",
        }, format="json", **headers)
        ...
        self.assertEqual(cleaning["calculation_basis"], "per_room_per_night")
        ...
        payload = self.payload()
        estimate = estimate_booking(payload)
        self.assertEqual(estimate["service_charge_total"], Decimal("42000"))
        self.assertEqual(estimate["tax_total"], Decimal("20000"))
        self.assertEqual(estimate["grand_total"], Decimal("382000"))

        booking, _ = create_booking(payload)
        invoice = booking.invoices.get(invoice_type=Invoice.Type.ROOM_BOOKING)
        self.assertEqual(booking.service_charge_total, Decimal("42000"))
        self.assertEqual(booking.tax_total, Decimal("20000"))
        self.assertEqual(booking.invoice_charge_snapshot["taxes"][0]["amount"], "0")
        self.assertEqual(booking.grand_total, Decimal("382000"))
        self.assertEqual(invoice.subtotal, Decimal("362000"))
        self.assertEqual(invoice.total, Decimal("382000"))
        self.assertEqual(
            list(invoice.lines.filter(metadata__line_type="invoice_service_charge").values_list("description", "total")),
            [("Service Charge", Decimal("32000")), ("Resort Fee", Decimal("10000"))],
        )

        self.hotel.refresh_from_db()
        self.hotel.ota_invoice_charges = {"taxes": [], "service_charges": []}
        self.hotel.save(update_fields=["ota_invoice_charges"])
        self.assertEqual(booking.invoice_charge_snapshot["taxes"][1]["mode"], "included")
        unconfigured_estimate = estimate_booking(payload)
        self.assertEqual(unconfigured_estimate["tax_total"], Decimal("0"))
        self.assertEqual(unconfigured_estimate["service_charge_total"], Decimal("0"))
        self.assertEqual(unconfigured_estimate["invoice_charges"], {"taxes": [], "service_charges": []})
    ...
    def test_admin_booking_detail_includes_hotel_check_times(self):
        ...
        booking_room.preference_snapshot = {
            ...
            "selected": {
                "bed": {"core_value_id": 12, "name": "King Bed"},
                "smoking": {"value": "non_smoking"},
                "custom_options": [
                    {"core_value_id": 31, "name": "High Floor"},
                    {"core_value_id": 32, "name": "Quiet Room"},
                ],
            },
        }
        booking_room.save(update_fields=["preference_snapshot"])
        ...
        self.assertEqual(
            [option["name"] for option in room_data["preference_details"]["custom_options"]],
    ...
    def test_bulk_ota_hotel_ids_returns_only_ota_sellable_candidates(self):
        PhysicalRoom.objects.create(
            hotel=self.hotel,
            room_type=self.room_type,
            core_physical_room_id=70000,
            room_number="OTA-ENABLED-1",
            ota_enabled=True,
            ota_sale_open=True,
        )
        response = self.client.post(
            "/api/v1/admin/ota/hotel-ids/",
            {"business_ids": [self.hotel.core_business_id, 999999]},
            format="json",
            HTTP_X_BOOKING_ADMIN_KEY="test-admin-key",
        )
        ...
        self.assertEqual(response.status_code, 200, response.data)
        self.assertEqual(
            response.data["data"]["ota_business_ids"],
            [self.hotel.core_business_id],
        )

    def test_bulk_hotel_availability_returns_only_available_candidate_ids(self):
        PhysicalRoom.objects.create(
            hotel=self.hotel,
            room_type=self.room_type,
            core_physical_room_id=70001,
            room_number="OTA-AVAILABLE-1",
            ota_enabled=True,
            ota_sale_open=True,
        )
        response = self.client.post(
        ...
        self.assertEqual(response.status_code, 200, response.data)
        self.assertEqual(
            response.data["data"]["available_business_ids"],
            [self.hotel.core_business_id],
        )
    ...
    def test_check_in_form_updates_same_partially_paid_pms_invoice_with_manual_items(self):
        ...
        response = self.client.patch(
            f"/api/v1/admin/bookings/{booking.id}/check-in-form/",
            {
                ...
                "invoice_adjustments[adjustment][mode]": "fixed",
                "invoice_adjustments[adjustment][value]": "2000",
        ...
        invoice.refresh_from_db()
        booking.refresh_from_db()
        self.assertEqual(response.data["data"]["invoice"]["id"], str(invoice.id))
        self.assertEqual(invoice.status, Invoice.Status.PARTIALLY_PAID)
        self.assertEqual(invoice.subtotal, Decimal("332000"))
        self.assertEqual(invoice.discount_total, Decimal("5000"))
        self.assertEqual(invoice.tax_total, Decimal("1000"))
        self.assertEqual(invoice.total, Decimal("328000"))
        self.assertEqual(invoice.balance, Decimal("327000"))
        self.assertEqual(booking.grand_total, Decimal("328000"))
        self.assertEqual(
            booking.invoices.filter(
                charge_scope=Invoice.ChargeScope.PMS,
        ...
        preserved = self.client.patch(
            f"/api/v1/admin/bookings/{booking.id}/check-in-form/",
            {"contact_name": "Updated PMS Guest"},
            format="multipart",
            **headers,
        )
    ...
    def test_check_in_form_keeps_paid_ota_invoice_locked_and_creates_pms_invoice(self):
        ...
        ota_invoice = booking.invoices.get(invoice_type=Invoice.Type.ROOM_BOOKING)
        record_payment(booking, {
            "invoice_id": ota_invoice.id,
            "payment_type": Payment.Type.FULL_PAYMENT,
            "provider": Payment.Provider.CASH,
            "amount": ota_invoice.total,
            "status": Payment.Status.PAID,
        }, auto_assign=False)
        ota_invoice.refresh_from_db()
        original_total = ota_invoice.total
        headers = {
            "HTTP_X_BOOKING_ADMIN_KEY": "test-admin-key",
            "HTTP_X_BOOKING_BUSINESS_ID": str(self.hotel.core_business_id),
        }
        ...
        self.assertEqual(response.status_code, 200, response.data)
        ota_invoice.refresh_from_db()
        self.assertEqual(ota_invoice.status, Invoice.Status.PAID)
        self.assertEqual(ota_invoice.total, original_total)
        pms_invoice = booking.invoices.get(charge_scope=Invoice.ChargeScope.PMS)
    ...
    def test_public_booking_flow_and_private_detail_token(self):
        ...
        self.assertEqual(detail.data["data"]["hotel"], {
    ...
    def test_room_board_aggregates_reserved_oos_and_unassigned_rooms(self):
        ...
        self.assertTrue(reserved["extra_bed_available"])
        self.assertEqual(reserved["extra_bed_quantity"], 1)
